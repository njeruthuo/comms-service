...
// Publisher is the subset of the messaging layer needed to enqueue an entry.
// *messaging.RabbitMQ satisfies it.
type Publisher interface {
	PublishJSON(queueName string, payload any) error
}

// Publish fills in sane defaults and pushes the entry onto queueName as JSON.
func Publish(p Publisher, queueName string, entry AuditLog) error {
	...
	return p.PublishJSON(queueName, entry)
}
